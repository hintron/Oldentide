...
**/
int SQLConnector::get_account_key(char *account_name, char *candidate_key){
    ...
    sqls = sqlite3_exec(database, query.str().c_str(), &get_account_key_callback, candidate_key, &error_message);
    if (sqls != SQLITE_OK){
        cout << "Could not execute SQL query! Return Code:" << sqls << endl;
    }
    else {
...
**/
// TODO: Make this a generic c string return callback
static int get_account_key_callback(void *key_string_hex, int argc, char **argv, char **azColName){
    int i;
    for(i = 0; i < argc; i++){
    ...
    strcpy((char *)key_string_hex, argv[0]);
    cout << "Canonized key string hex from db: " <<  key_string_hex << endl;
    return 0;
}
